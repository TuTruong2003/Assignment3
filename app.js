...
const Planet = (prop) => {
  return (
    <div className="card">
      ...
      <ul>
        <li><strong>Diameter:</strong> { prop.diameter }</li>
        <li><strong>Moons:</strong> { prop.moons }</li>
      </ul>

      <button onClick={() => prop.handleRemove(prop.id)}>Remove Planet</button>
    </div>
  );
}
...
state ={
    listOfPlanet: planets
   };

  removePlanet = (id) => {
    this.setState(prevState => {
      return {
        listOfPlanet: prevState.listOfPlanet.filter(p => p.id !== id)
      };
  ...
  render() {
    return (
      <div className="container">
        ...
        {this.state.listOfPlanet.map(planet =>
          <Planet
            id={planet.id}
            name={planet.name}
            ...
            desc={planet.desc}
            moons={planet.moons}
            handleRemove={this.removePlanet}
            url={planet.url}
